...
//                DefaultScrollHandle scrollHandle = new DefaultScrollHandle(context);
                config.scrollHandle(new PDFScrollHandle(context, enable -> {
                    Map<String, Object> args = new HashMap<>();
                    args.put("enable", enable);
                    methodChannel.invokeMethod("onScrollHandlerChanged", args);
                }));
            }
        }
    }
